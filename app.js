...
function proratedBase(annual, period, emp) {
  ...
  const countDays = (a, b) => {
    let n = 0;
    for (let d = parseDate(a); fmtISO(d) <= b; d = addDays(d, 1)) if (!weekdaysOnly || d.getDay() >= 1 && d.getDay() <= 5) n++;
    return n;
  };
...
function periodIndexFor(iso) {
  const anchor = parseDate(PERIOD_ANCHOR_START);
  const target = parseDate(iso);
  const diffDays = Math.floor((target - anchor) / 864e5);
  return Math.floor(diffDays / PERIOD_LEN_DAYS);
}
function periodByIndex(i) {
  const start = addDays(parseDate(PERIOD_ANCHOR_START), i * PERIOD_LEN_DAYS);
  const end = addDays(start, PERIOD_LEN_DAYS - 1);
  const payday = addDays(end, PAYDAY_OFFSET_DAYS);
  const lockAt = new Date(payday);
  lockAt.setHours(0, 0, 0, 0);
  lockAt.setHours(lockAt.getHours() - LOCK_LEAD_HOURS);
  return {
    index: i,
    start: fmtISO(start),
    end: fmtISO(end),
...
async function loadEntriesForUid(uid) {
  try {
...
async function saveEntriesForUid(uid, val) {
  try {
    const ref = window._fs.doc(window._db, "paytracker_entries", uid);
...
async function loadCertsForUid(uid) {
  try {
    const ref = window._fs.doc(window._db, "paytracker_entries", uid);
    const snap = await window._fs.getDoc(ref);
...
async function saveCertForUid(uid, periodIdx, cap, atISO) {
  try {
    const ref = window._fs.doc(window._db, "paytracker_entries", uid);
...
async function loadSalaries() {
  try {
    const ref = window._fs.doc(window._db, "paytracker_entries", SALARY_DOC);
    const snap = await window._fs.getDoc(ref);
...
async function loadMySalary(uid) {
  try {
    const ref = window._fs.doc(window._db, "paytracker_entries", uid);
    const snap = await window._fs.getDoc(ref);
...
async function saveMySalary(uid, annual) {
  try {
    const ref = window._fs.doc(window._db, "paytracker_entries", uid);
...
async function loadAdjustments() {
  try {
    const ref = window._fs.doc(window._db, "paytracker_entries", ADJ_DOC);
    const snap = await window._fs.getDoc(ref);
...
async function saveAdjustments(map) {
  try {
    const ref = window._fs.doc(window._db, "paytracker_entries", ADJ_DOC);
    await window._fs.setDoc(ref, { value: map });
    return true;
...
async function saveMyAdj(uid, map) {
  try {
    const ref = window._fs.doc(window._db, "paytracker_entries", uid);
...
async function loadPublicUsernames() {
  try {
    ...
    const snap = await window._fs.getDoc(ref);
    if (snap.exists() && snap.data() && Array.isArray(snap.data().value)) return snap.data().value;
    return null;
  } catch (e) {
    return null;
  }
}
async function savePublicUsernames(list) {
  try {
    const ref = window._fs.doc(window._db, "public", "usernames");
    await window._fs.setDoc(ref, { value: list });
    return true;
  } catch (e) {
...
async function loadPublicRoster() {
  try {
    ...
    return { usernames: d && Array.isArray(d.value) ? d.value : null, dir: d && d.dir || {}, managed: d && Array.isArray(d.managed) ? d.managed : [] };
  } catch (e) {
...
function App() {
  ...
  useEffect(() => {
    ...
    return window._authfns.onAuthStateChanged(window._auth, async (u) => {
      setAuthUser(u || null);
      await refresh(u);
      setLoaded(true);
    });
  ...
  const deleteOwnerEntry = useCallback(async (entry) => {
    ...
    await saveEntriesForUid(entry._uid, latest.filter((e) => e.id !== entry.id));
  ...
  if (!loaded) return /* @__PURE__ */ React.createElement("div", { className: "wrap" }, /* @__PURE__ */ React.createElement("div", { className: "empty" }, "Loading\u2026"));
  return /* @__PURE__ */ React.createElement("div", { className: "wrap" + (isOwner && !impersonate ? " wide" : "") }, /* @__PURE__ */ React.createElement("div", { className: "brand-header" }, /* @__PURE__ */ React.createElement("img", { className: "brand-logo", src: "data:image/png;base64,", alt: "Houston Renal Group" })), authUser && /* @__PURE__ */ React.createElement("div", { className: "topbar" }, /* @__PURE__ */ React.createElement("h1", null, "Pay Tracker"), (isOwner || myEmp) && /* @__PURE__ */ React.createElement("button", { className: "btn btn-ghost", style: { fontSize: 13, padding: "6px 12px" }, onClick: signOutNow }, "Sign out")), !authUser && /* @__PURE__ */ React.createElement(LoginScreen, { showToast }), isOwner && impersonate && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { className: "impersonate-bar" }, /* @__PURE__ */ React.createElement("span", null, "\u{1F441} Viewing as ", /* @__PURE__ */ React.createElement("strong", null, lastFirst(impersonate.name)), impersonate.isManager ? " (office manager) \u2014 their staff page" : " \u2014 edits save to their record (audit)."), /* @__PURE__ */ React.createElement("button", { className: "btn", onClick: exitImpersonate }, "Exit view")), impersonate.isManager ? /* @__PURE__ */ React.createElement(
    ManagerView,
    {
      key: impersonate.id,
      manager: impersonate,
...
function EntryView({ emp, entries, upsertEntry, certs, certifyPeriod, manualLocks, manualUnlocks, showToast, audit, baseSalary, empAdj, pto, ptoAllowance, ptoStartDate, onRequestPto, onCancelPto }) {
  ...
  }, disabled: periodIdx >= maxPeriodIdx, "aria-label": "Next pay period" }, "\u203A")), /* @__PURE__ */ React.createElement("div", { className: "period-total" }, baseBiweekly > 0 || periodBonus > 0 || periodReimb > 0 || periodStipend > 0 ? /* @__PURE__ */ React.createElement("div", { className: "pay-breakdown" }, baseBiweekly > 0 && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("span", null, "Base", baseProrated ? " (prorated " + basePr.days + "/" + basePr.of + (basePr.of === PERIOD_LEN_DAYS ? " days)" : " weekdays)") : ""), /* @__PURE__ */ React.createElement("span", { className: "pay" }, money(baseBiweekly))), (!noPayTypes || periodDollars > 0) && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("span", null, "Variable"), /* @__PURE__ */ React.createElement("span", { className: "pay" }, money(periodDollars))), periodBonus > 0 && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("span", null, "Bonus"), /* @__PURE__ */ React.createElement("span", { className: "pay" }, money(periodBonus))), periodReimb > 0 && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("span", null, "Reimbursement"), /* @__PURE__ */ React.createElement("span", { className: "pay" }, money(periodReimb))), periodStipend > 0 && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("span", null, emp && String(emp.stipendNote || "").trim() || "Stipend"), /* @__PURE__ */ React.createElement("span", { className: "pay" }, money(periodStipend))), /* @__PURE__ */ React.createElement("div", { className: "pay-total" }, /* @__PURE__ */ React.createElement("span", null, "Total"), /* @__PURE__ */ React.createElement("span", { className: "pay" }, money(baseBiweekly + periodDollars + periodBonus + periodReimb + periodStipend)))) : /* @__PURE__ */ React.createElement("span", null, "This period:", /* @__PURE__ */ React.createElement("span", { className: "pay", style: { marginLeft: 6 } }, money(periodDollars))), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "var(--muted)", marginTop: 4 } }, "Payday ", period.paydayLabel))), !noPayTypes && /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { height: 12 } }), /* @__PURE__ */ React.createElement("div", { className: "week-grid" }, DOW.map((d) => /* @__PURE__ */ React.createElement("div", { className: "dow", key: "dow-" + d }, d)), periodDays.map((iso) => {
  ...
  useEffect(() => {
    ...
    try {
      if (sortKey) {
        ...
        localStorage.setItem("hrgRollupSortDir", sortDir);
      } else {
        localStorage.removeItem("hrgRollupSortKey");
        localStorage.removeItem("hrgRollupSortDir");
      }
    } catch (e) {
    }
  }, [sortKey, sortDir]);
...
function Rates({ employees, salaries, persistEmployees, persistSalaries, showToast }) {
  ...
  ), Number(emp.annualSalary) > 0 && /* @__PURE__ */ React.createElement("div", { className: "fixed-note", style: { marginTop: 4 } }, money(Number(emp.annualSalary) / 26), " per full period \xB7 prorated around a start / last day by weekdays (W-2) or calendar days (1099)"))), /* @__PURE__ */ React.createElement("div", { className: "field-row" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("label", null, "PTO days ", /* @__PURE__ */ React.createElement("span", { className: "hint-sm" }, "per calendar year \xB7 resets Jan 1 \xB7 blank = none")), /* @__PURE__ */ React.createElement(
    "input",
    {
      type: "text",
      inputMode: "decimal",
